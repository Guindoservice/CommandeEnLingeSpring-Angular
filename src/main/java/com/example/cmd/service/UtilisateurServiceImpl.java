...
//        }
        ...
        try {
            ...
            return utilisateurRepository.save(personnel);
        } catch (Exception e) {
    ...
    @Transactional
    public String modifierUtilisateur(String username, String nouveauNom, String nouveauPrenom) {
        Utilisateur utilisateur = utilisateurRepository.findByUsername(username)
                .orElseThrow(() -> new RuntimeException("Utilisateur non trouvé: " + username));

        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        if (!authentication.getName().equals(username)) {
            throw new IllegalStateException("Vous n'êtes pas autorisé à modifier cet utilisateur");
        }
        ...
        return "Informations utilisateur mises à jour avec succès pour l'utilisateur: " + username;
    }
